<script lang="ts">
	...
	onMount(() => {
		...
		successMessage = urlParams.get('success') === 'true' ? 'Poprawnie dodano użytkownika' : '';
	});
	...
	function validate(event: Event) {
		...
		if (!fingerID || fingerID.length < 6) {
			errors.fingerID = 'Kod Odcisku Palca musi mieć co najmniej 6 znaków.';
		}
...
<main class="flex flex-col items-center justify-center h-screen bg-gray-100 p-6">
	...
	{/if}
	<form class="flex flex-col gap-4 p-8 bg-white rounded-lg shadow-md w-full max-w-3xl" method="post" use:enhance on:submit={validate}>
		<!-- Imie i Nazwisko -->
		<div class="flex gap-4">
			<div class="flex-1">
				<label for="imie" class="block mb-2">Imie:</label>
				<input type="text" id="imie" name="imie" required class="w-full p-2 border border-gray-300 rounded" />
				{#if errors.imie}
		...
		<div class="flex gap-4">
			<div class="flex-1">
				...
				<select name="ranga" id="ranga" class="w-full p-2 border border-gray-300 rounded">
		...
		<div class="flex gap-4">
			...
			<div class="flex-1">
				...
				<input type="text" id="fingerID" name="fingerID" class="w-full p-2 border border-gray-300 rounded" />
				{#if errors.fingerID}
		...
		<button type="submit" class="w-full py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Zarejestruj się</button>
	</form>
</main>
